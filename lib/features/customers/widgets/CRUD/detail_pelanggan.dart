...
class DetailPelangganPopup extends StatelessWidget {
  ...
  final formattedRiwayat = transaksiResp.map((trx) {
    ...
    return {
      'id': trx['no_transaksi'] ?? 'TRX-${trx['id']}'.substring(0, 8),
      'tanggal': DateTime.parse(trx['tanggal_waktu'] as String),
  ...
  final stats = {
    ...
    'total_belanja': NumberFormat.currency(locale: 'id_ID', symbol: '', decimalDigits: 0)
            .format(totalBelanja.round()) +
        'k',
    'rata_rata': NumberFormat.currency(locale: 'id_ID', symbol: '', decimalDigits: 0)
  ...
  Widget _buildCustomerCard(Map<String, dynamic> pelangganData) {
    final pelanggan = pelangganData['pelanggan'] as Map<String, dynamic>;

    final String initials = pelanggan['nama']
        .toString()
        .split(' ')
    ...
    final String joinedDate = pelanggan['created_at'] != null
        ? DateFormat(
            'dd MMMM yyyy',
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "Tanggal: ${DateFormat('dd MMMM yyyy, HH:mm').format(trx['tanggal'])}",
              style: const TextStyle(
                fontSize: 12,
                color: Color.fromRGBO(107, 79, 63, 1),
              ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: const Color(0xFF4DB36C)),
              ),
              child: Text(
                trx['status'],
                style: const TextStyle(
                  color: Color(0xFF4DB36C),
